...
from backend.utils.image_validation import ImageModel
...
class TumorDetectionService:
    ...
    def process_and_predict(self, file_io):
        ...
        """
        try:
            # Validate uploaded bytes are a JPEG before further processing
            file_io.seek(0)
            content = file_io.read()
            try:
                ImageModel(data=content)
            except ValueError as e:
                logger.error(f"Image validation failed: {e}")
                raise
            # Reset stream for downstream readers
            file_io.seek(0)
